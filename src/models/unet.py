...
from src.models.modules.attention import LinearAttention, Attention
from src.models.modules.misc import Residual, SinusoidalPosEmb, LearnedSinusoidalPosEmb
...
class Unet(BaseModel):
    def __init__(
            ...
            learned_sinusoidal_dim=16, **kwargs
    ):
        ...
        input_channels = input_channels * (2 if self.hparams.conditioned else 1)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                block_klass(dim_in, dim_in, time_emb_dim=time_dim),
                Residual(PreNorm(dim_in, fn=LinearAttention(dim_in, rescale='qkv'), norm=LayerNorm)),
                Downsample(dim_in, dim_out) if not is_last else nn.Conv2d(dim_in, dim_out, 3, padding=1)
            ]))

        mid_dim = dims[-1]
        self.mid_block1 = block_klass(mid_dim, mid_dim, time_emb_dim=time_dim)
        self.mid_attn = Residual(PreNorm(mid_dim, fn=Attention(mid_dim), norm=LayerNorm))
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim),
                Residual(PreNorm(dim_out, fn=LinearAttention(dim_out, rescale='qkv'), norm=LayerNorm)),
