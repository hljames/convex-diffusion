...
from src.utilities.wandb_api import get_wandb_ckpt_name, load_hydra_config_from_wandb, \
...
def reload_and_test_model(run_id: str,
    ...
    if config is None:
        config = load_hydra_config_from_wandb(run_path, override_key_value=override_kwargs)
    ...
    if train_model:
        x = trainer.fit(model=model, datamodule=datamodule)

    if test_model:  # Testing:
        x = trainer.test(datamodule=datamodule, model=model)
