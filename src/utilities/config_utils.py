...
from src.utilities.naming import get_group_name, get_detailed_name, clean_name
...
def extras(config: DictConfig) -> None:
    ...
    if False: #accelerator in ["ddp", "ddp_spawn", "dp", "ddp2"]:
    ...
    USE_WANDB = "logger" in config.keys() and config.logger.get("wandb") and hasattr(config.logger.wandb, "project")
    if USE_WANDB:
        if not config.logger.wandb.get('id'):
            ...
            if 'SLURM_JOB_ID' in os.environ:
                ...
                config.logger.wandb.id = wandb.util.generate_id()
        else:
        ...
        suffix = '_' + time.strftime('%Hh%Mm_on_%b_%d') + '_' + config.logger.wandb.id
        if not config.logger.wandb.get('name'):  # no wandb name has been assigned yet
            config.logger.wandb.name = get_detailed_name(config, add_prefix=True) + suffix
        ...
        for _, module_config in config[module_name].items():
            if module_config is not None and "_target_" in module_config:
                try:
                    ...
                    modules.append(hydra.utils.instantiate(module_config, settings=wandb.Settings(start_method='fork')))
...
def log_hyperparameters(
    ...
    params["datamodule"] = copy_and_ignore_keys(config["datamodule"], 'pin_memory')
    ...
    params['dirs/wandb_save_dir'] = config.logger.wandb.get('save_dir') if (
...
def get_model_from_hydra_compose_overrides(overrides: List[str]):
    ...
    """
    from src.interface import get_model
